...
* **10.2 Usage** — `[to complete]` (only the screen-reader check)
  ...
  * `[done]` `NO_COLOR`. The variable being **present** is the request, whatever it is
    ...
    colours go.
  * `[to complete]` The screen-reader check. Not something to tick off from here: it
    means running VOX under NVDA, JAWS, VoiceOver or Orca and listening to what they
    say, and the answer is a list of specific fixes that cannot be guessed. Textual has
    some support of its own, and the things most likely to be wrong are already visible
    from the code — the live-updating status bar and spinner announce constantly, the
    transcript has no landmark structure, and the modals do not set an accessible name.
    Someone who uses a screen reader daily would find more in ten minutes than a
    checklist would in a day.
